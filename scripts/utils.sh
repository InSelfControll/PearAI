...
configure_permissions() {
    ...
    if [ "$IS_NIXOS" = true ]; then
        # For NixOS, we only change permissions, not ownership
        if [ "$type" = "dir" ]; then
            chmod 755 "$target" || handle_error "Failed to set permissions on directory $target"
        else
            chmod 644 "$target" || handle_error "Failed to set permissions on file $target"
        fi
    else
        # For non-NixOS, we change both ownership and permissions
        chown "$REAL_USER:$(id -gn $REAL_USER)" "$target" || handle_error "Failed to change ownership of $target"
        if [ "$type" = "dir" ]; then
            chmod 755 "$target" || handle_error "Failed to set permissions on directory $target"
        else
            chmod 644 "$target" || handle_error "Failed to set permissions on file $target"
